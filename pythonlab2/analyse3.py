...
def analyze_execution_time(task_data):
    stats = task_data.groupby("Lambda").agg(
        Mean_Ti=("Total Execution Time", "mean"),
        Std_Ti=("Total Execution Time", "std")
    ).reset_index()
    return stats

def save_analysis_with_chart(stats, output_file="execution_time_analysis_lambda.xlsx"):
    wb = Workbook()
    ws = wb.active
    ws.title = "Execution_Time_Stats_Lambda"

    ws.append(["Lambda", "Mean Ti", "Std Ti"])
